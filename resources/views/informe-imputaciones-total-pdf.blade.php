...
$pdf_color_trabajo = '#74aaf8';

if (!function_exists('tcTotal')) {
    function tcTotal($nombre, $map) {
...
$pdf_meses = ['','Enero','Febrero','Marzo','Abril','Mayo','Junio',
              'Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];

$pdf_total = count($usuarios_data);
@endphp

@foreach($usuarios_data as $pdf_idx => $ud)
@php
$usuario          = $ud['usuario'];
$dias             = $ud['dias'];
$tipos            = $ud['tipos'];
...
<div style="{{ $es_ultimo ? '' : 'page-break-after:always' }}">
    ...
    <div class="pdf-header">
        <div class="pdf-header-left">
            ...
            <div class="pdf-period">{{ $pdf_meses[$month] }} {{ $year }} &mdash; {{ $usuario->nombre ?? '' }}</div>
        </div>
    </div>
        ...
        <div class="col-left">
            ...
            <div class="panel">
                ...
                <table class="tbl-small">
                    <thead><tr><th>Mes</th><th style="color:#1a7a34">+</th><th style="color:#cc2200">-</th><th>Total</th></tr></thead>
                    <tbody>
            ...
            <div class="panel">
                ...
                <table class="tbl-small">
                    <thead><tr>
                        <th>Mes</th>
                        <th style="color:#74aaf8">T</th><th style="color:#e83e8c">C</th>
                        <th style="color:#e8b800">V</th><th style="color:#7b3f8c">B</th>
                        <th style="color:#34c163">AA</th><th>S</th><th style="color:#999">Lab.</th>
                    </tr></thead>
                    <tbody>
                    @foreach($year_stats as $s)
                        @if($s['total_dias'] > 0)
                        <tr>
                            <td>{{ $s['label'] }}</td>
                            <td>{{ $s['dias_col']['T'] ?: '' }}</td><td>{{ $s['dias_col']['C'] ?: '' }}</td>
                            <td>{{ $s['dias_col']['V'] ?: '' }}</td><td>{{ $s['dias_col']['B'] ?: '' }}</td>
                            <td>{{ $s['dias_col']['AA'] ?: '' }}</td><td>{{ $s['total_dias'] }}</td>
